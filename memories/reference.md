...
|---|---|---|
| Google OAuth token (Gmail + Calendar + Sheets + Drive) | `credentials/google_token.json` | Generate once with `python setup/google_auth_setup.py`. Auto-refreshes. Same Google account as MCP (vinh.nguyen@inapps.net). |
...
| Google Sheets | vinh.nguyen@inapps.net | — | `google-api-python-client` + `credentials/google_token.json` | Python only; skill: `skills/Google-Sheet/skill.md` |
...
|---|---|---|
| `setup/google_auth_setup.py` | Generate `credentials/google_token.json` | Once before any Python Gmail/Calendar/Sheets script |
